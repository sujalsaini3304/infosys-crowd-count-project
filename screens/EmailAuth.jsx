...
export default function EmailAuth() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // If verified, create user via API
      ...
      setMessage({
        text: result.message || "Signup successful! Redirecting...",
        type: "success",
      });
  ...
  const handleResend = async () => {
    ...
    try {
      const response = await axios.post(`${server}/api/send/auth/email`, {
        username,
        email,
      });
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-lg shadow-md p-8">
          ...
          <button
            ...
            className="w-full bg-blue-600 text-white py-3 px-4 rounded-md font-medium hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:bg-blue-400 disabled:cursor-not-allowed transition-colors flex items-center justify-center mb-4"
          >
            {isLoading ? (
              <>
